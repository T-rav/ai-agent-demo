...
from ..models import IngestionConfig, ConfigurationError
...
class Config:
    ...
    def get_processing_config(self) -> Dict[str, int]:
        ...
        return {
            ...
            'embedding_batch_size': int(os.getenv('EMBEDDING_BATCH_SIZE', str(processing_config.get('embedding_batch_size', 100)))),
            'upsert_batch_size': int(os.getenv('UPSERT_BATCH_SIZE', str(processing_config.get('upsert_batch_size', 100))))
        }
    ...
    def get_logging_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'show_progress': os.getenv('SHOW_PROGRESS', str(logging_config.get('show_progress', True))).lower() == 'true'
        }
    ...
    def validate_required_config(self) -> None:
        ...
        for field, env_var in required_fields:
            ...
            if not value or value == f'your_{field}_here':
                missing_fields.append(f"{field} (set via {env_var} environment variable)")
